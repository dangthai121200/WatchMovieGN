...
public class ConvertUtil {
    ...
    public static CreateMovieRequest convertMovieToCreateMovieRequest(Movie movie) {
        CreateMovieRequest createMovieRequest = modelMapper.map(movie, CreateMovieRequest.class);
        return createMovieRequest;
    }

    public static List<Movie> convertListMovieDomainToListMovie(List<MovieDomain> movieDomains) {
    ...
    public static List<CreateMovieRequest> convertMovieToListCreateMovieRequest(Set<Movie> movies) {
        if (!movies.isEmpty()) {
            return movies.stream().map(ConvertUtil::convertMovieToCreateMovieRequest).collect(Collectors.toList());
        }
        return Collections.emptyList();
    }
